...
export default function Movie() {
  ...
  const handleClick = async() => {
	  let urlPara;
	  if(fname || searchParams.get("m"))
	  {
		let urlPara = (fname)?fname:searchParams.get("m");
	  
	  const url = "https://search.imdbot.workers.dev/?q="+urlPara;
	  console.log(urlPara,"==url=========",url);
		//setFname("");
		try {
			...
			if(response) {
				...
				setResponseData(allUsers);
				console.log(responseData);
			}
			//navigate('/movie/?m='+(fname)?fname:searchParams.get("m"));
			 navigate({
			  pathname: '/movie/',
			  search: '?m='+urlPara,
			});
